...
pub(crate) fn derive_native_class(derive_input: &DeriveInput) -> Result<TokenStream, syn::Error> {
    derive_native_class2(derive_input).map(TokenStream::from)
}

fn derive_native_class2(derive_input: &DeriveInput) -> Result<TokenStream2, syn::Error> {
    let derived = crate::automatically_derived();
...
mod tests {
    ...
    fn check_property_combination(
        ...
        let derived = derive_native_class2(&derive_input);
